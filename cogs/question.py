...
category_id = 874472288907579422
close_ticket_category_id = 874472792681226271
# 오류 ~~~~ 37라인
...
class question(commands.Cog): # 야이 미친놈아 command.Cog가 뭐냐
    ...
    async def on_message(self, message: discord.Message):
        ...
        if str(message.channel.type) in "private":
            ...
            try:
                ...
                pass
    # @commands.command(name="문의오픈", aliases=["티켓오픈", "close"])
    # @commands.has_permissions(administrator=True)
    # async def ticket_open(self, ctx, user_id:int):
    #             open_ticket_category = ctx.guild.get_channel(category_id) # 이건 왜 길드 아이디지
    #             user = await self.bot.fetch_user(user_id)
    #             new_ticket = await user_id.send(
    #                         embed = discord.Embed(
    #                         title=f'문의',
    #                         description =f"관리자로 부터 티켓이 열렸습니다." ,
    #                         colour = discord.Colour.blue(),
    #                     ).set_thumbnail(url="https://cdn.discordapp.com/app-icons/872714206246469662/b2cf639a78e58b0af178a980764d472d.png?size=256")                )

    #             ticket_channel = await open_ticket_category.create_text_channel(
    #                 f'{user.dm_channel.id}({user})',
    #                 topic=f"{user_id}",
    #                 position = 1
    #             )
    #             staff = ctx.guild.get_role(857395557793792020) # 이건 안넣냐
    #             await ticket_channel.set_permissions(
    #                 staff,
    #                 read_messages=True,
    #                 send_messages=True,
    #                 read_message_history=True,
    #             )
    #             m=await ctx.send(f"{user}티켓이 열렸습니다")
    ...
    async def ticket_end(self, ctx):
        ...
        await (await self.bot.fetch_user(int(ctx.channel.topic))).send(
            ...
            colour = discord.Colour.blue(),
            ).set_thumbnail(url="https://cdn.discordapp.com/avatars/873899327758692352/810ef9d933f9985d82f441de0a03fb6b.webp?size=80")
